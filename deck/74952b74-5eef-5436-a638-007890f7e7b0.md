...
claim_meta:
  verdict: NOT_FOUND
  explanation: "The Reference Text confirms the dbt_project.yml fields on-run-start and on-run-end, their values as single SQL statements or lists, and provides an example of using on-run-end for grant statements with Jinja. However, it does not explicitly state that GRANT statements were the most common use in older versions or mention the dbt v1.2 changes to the 'grants' config, so this part of the Answer is not verified."
  citation:
    evidence_source: "https://docs.getdbt.com/reference/project-configs/on-run-start-on-run-end"
