...
claim_meta:
  verdict: NOT_FOUND
  explanation: "The reference text explicitly supports the identification of the fields `on-run-start` and `on-run-end` in dbt_project.yml. However, it does not contain information about the most common historical use of this functionality (e.g., GRANT statements) or the update in dbt v1.2 with the 'grants' option. Therefore, the answer cannot be fully verified against the provided reference text."
  citation:
    evidence_source: "https://docs.getdbt.com/reference/dbt_project.yml"
