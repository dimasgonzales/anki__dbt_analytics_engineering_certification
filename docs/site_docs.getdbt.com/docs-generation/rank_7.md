# Source: https://docs.getdbt.com/blog/star-sql-love-letter

A star (generator) is born | dbt Developer Blog
...
[dbt Docs](/ "dbt Docs")[Developer Blog](/blog "Blog")[A star (generator) is born](# "A star (generator) is born")
...
[![Kira Furuichi](/img/blog/authors/kira-furuichi.png)](/blog/authors/kira-furuichi)

[Kira Furuichi](/blog/authors/kira-furuichi)
...
The `star` dbt macro[​](#the-star-dbt-macro "Direct link to the-star-dbt-macro")
--------------------------------------------------------------------------------

dbt supports [dbt\_utils](https://github.com/dbt-labs/dbt-utils), a [package of macros and tests](https://docs.getdbt.com/docs/build/packages) that data folks can use to help them write more DRYDRY is a software development principle that stands for “Don’t Repeat Yourself.” Living by this principle means that your aim is to reduce repetitive patterns and duplicate code and logic in favor of modular and referenceable code. code in their dbt project. One of the macros dbt utils offers is the `star` generator.
...
[Under the hood](https://github.com/dbt-labs/dbt-utils/blob/main/macros/sql/star.sql), the `star` macro is actually using another dbt utils macro ([get\_filtered\_columns\_in\_relation](https://github.com/dbt-labs/dbt-utils#get_filtered_columns_in_relation-source)) to loop through fields to either select, alias, and/or append some string values to them.

Why we love the `star` macro[​](#why-we-love-the-star-macro "Direct link to why-we-love-the-star-macro")
--------------------------------------------------------------------------------------------------------
...
![](/assets/images/utils-madness-1-afcc9d6de83a8f8f1e52ea2e6fa47ded.png)
...
Tackling the complexity of joining snapshots](/blog/joining-snapshot-complexity)[Older post

Optimizing dbt Models with Redshift Configurations](/blog/redshift-configurations-dbt-model-optimizations)

* [The `star` dbt macro](#the-star-dbt-macro)
* [Why we love the `star` macro](#why-we-love-the-star-macro)
