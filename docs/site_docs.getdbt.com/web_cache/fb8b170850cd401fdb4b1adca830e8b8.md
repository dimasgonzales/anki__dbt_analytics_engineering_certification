...
dbt-databricks v1.9 adds support for the `table_format: iceberg` config. Try it now on the [dbt "Latest" release track](/docs/dbt-versions/cloud-release-tracks). All other table configurations were also supported in 1.8.
...
In dbt-databricks v1.9 (try it now in [the dbt "Latest" release track](/docs/dbt-versions/cloud-release-tracks)), you can use these four options for `submission_method`:
...
* `insert_overwrite`: If `partition_by` is specified, overwrite partitions in the tableIn simplest terms, a table is the direct storage of data in rows and columns. Think excel sheet with raw values in each of the cells. with new data. If no `partition_by` is specified, overwrite the entire table with new data.
...
To configure this inside of dbt, use the [extended attributes feature](/docs/dbt-cloud-environments#extended-attributes-) on the desired environments:
...
We support [on\_configuration\_change](/reference/resource-configs/on_configuration_change) for most available properties of these materializations.
