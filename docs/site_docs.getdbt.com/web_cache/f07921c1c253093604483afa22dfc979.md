About modules variable | dbt Developer Hub
...
Starting in `dbt-core==1.10.6`, using `modules.itertools` raises a deprecation warning. For more information and suggested workarounds, refer to the [documentation on `ModulesItertoolsUsageDeprecation`](/reference/deprecations#modulesitertoolsusagedeprecation).
...
model](/reference/dbt-jinja-functions/model)[Next
